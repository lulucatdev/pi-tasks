...
const root = await fs.mkdtemp(path.join(os.tmpdir(), "pi-job-smoke-"));
...
const result = await executeSupervisedJobs({
  ...
  runAttempt: async (input) => {
    const scenario = scenarios.get(input.job.name);
    await fs.mkdir(input.paths.attemptDir, { recursive: true });
    await fs.writeFile(input.paths.workerLogPath, `${scenario} log`, "utf-8");
    if (scenario === "transient") {
      return { attemptId: input.attemptId, jobId: input.job.id, status: "error", exitCode: 1, sawTerminalAssistantMessage: false, stderrTail: "500 Internal server error", stdoutMalformedLines: 0, failureKind: "unknown", error: "500 Internal server error", startedAt: "start", finishedAt: "finish" };
    }
    if (scenario === "invalid") {
      await fs.writeFile(input.paths.reportPath, "{bad", "utf-8");
    } else {
      await fs.writeFile(input.paths.reportPath, JSON.stringify({ schemaVersion: 1, jobId: input.job.id, attemptId: input.attemptId, status: scenario === "blocked" ? "blocked" : "completed", summary: `${scenario} summary`, deliverables: [], evidence: [] }), "utf-8");
    }
    return { attemptId: input.attemptId, jobId: input.job.id, status: "success", exitCode: 0, sawTerminalAssistantMessage: true, stderrTail: "", stdoutMalformedLines: 0, failureKind: "none", error: null, startedAt: "start", finishedAt: "finish" };
  },
});
...
console.log(`job audit smoke ok: ${result.batch.batchDir}`);
NODE
