...
import { buildAttemptPaths, buildBatchPaths } from "../../extensions/jobs/audit-log.ts";
import { runWorkerAttempt } from "../../extensions/jobs/worker-runner.ts";
import { appendWorkerEvent, buildWorkerEvent, createStdoutTelemetryState, extractStdoutTelemetry, readWorkerEvents, workerEventsPathForAttempt } from "../../extensions/jobs/worker-events.ts";
...
test("extractStdoutTelemetry emits tool_call and file_write_observed events for write tools", () => {
  ...
  const ctx = { jobId: "t001", attemptId: "t001-a1", cwd: "/tmp/project" };
...
test("runWorkerAttempt creates worker event channel path and exposes env", async () => {
  ...
  await runWorkerAttempt({
    job: { id: "t001", name: "demo", prompt: "Do it", cwd: process.cwd() },
    attemptId: "t001-a1",
    attemptIndex: 1,
    paths,
  ...
  assert.equal(seenEnv.PI_CHILD_TYPE, "job-worker");
  assert.equal(seenEnv.PI_JOB_EVENTS_PATH, workerEventsPathForAttempt(paths.attemptDir));
