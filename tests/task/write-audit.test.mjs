...
import { buildWorkerEvent, observedWritePaths } from "../../extensions/task/worker-events.ts";

test("observedWritePaths extracts file write telemetry paths without raw args", () => {
  const events = [
    buildWorkerEvent({ type: "file_write_observed", taskId: "t001", attemptId: "t001-a1", path: "allowed/out.md", args: { token: "secret" } }),
    buildWorkerEvent({ type: "progress", taskId: "t001", attemptId: "t001-a1", message: "working" }),
  ];
