...
export type JobLifecycleStatus = "queued" | "running" | "success" | "error" | "aborted";
export type JobFinalStatus = "success" | "error" | "aborted";
...
export type JobActivityKind = "thinking" | "tool";
...
export interface JobsToolParams {
  jobs: JobSpecInput[];
  concurrency?: number;
...
export interface NormalizedJobSpec extends JobSpecInput {
  id: string;
  cwd: string;
  prompt: string;
  acceptance?: AcceptanceContract;
...
export interface JobArtifact {
  ...
  finishedAt: string | null;
  timeline: Array<{ at: string; state: JobLifecycleStatus; message?: string }>;
...
export interface DeriveJobStatusInput {
...
export function deriveJobFinalStatus(input: DeriveJobStatusInput): JobFinalStatus {
