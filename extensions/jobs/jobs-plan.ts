/**
 * jobs_plan: compact local fan-out expansion.
 *
 * The model emits a small `matrix + promptTemplate + acceptanceTemplate`
 * payload. The extension expands it locally into a full
 * internal `JobsToolParams` and hands it to the existing supervisor. This avoids the
 * "model streams 19 huge prompts as one mega tool-call argument" failure mode
 * that prevents `jobs.execute()` from ever starting.
 */

import * as fs from "node:fs/promises";
import * as path from "node:path";
import type { AcceptanceContract, ParentRetryPolicy, PathCheck, JobSpecInput, JobsToolParams, ThrottlePolicy } from "./types.ts";
...
export interface JobsPlanSynthesis {
	mode?: "parent" | "report-only";
	instructions?: string;
}

export interface JobsPlanInput {
	batchName: string;
	concurrency?: number;
	matrix: JobsPlanRow[];
...
export interface ExpandedJobsPlan {
...
export function validateJobsPlanInput(input: unknown): asserts input is JobsPlanInput {
	assertObject(input, "jobs_plan params must be an object.");
	const record = input as Record<string, unknown>;
	for (const key of Object.keys(record)) {
		if (!ALLOWED_PLAN_KEYS.has(key)) throw new Error(`jobs_plan.${key} is not supported. jobs_plan is frozen to matrix/template transport; move workflow logic into worker prompts or parent synthesis.`);
	}
	assertString(record.batchName, "jobs_plan.batchName must be a non-empty string.");
	assertString(record.promptTemplate, "jobs_plan.promptTemplate must be a non-empty string.");
	if (Buffer.byteLength(record.promptTemplate as string, "utf-8") > MAX_PLAN_PROMPT_TEMPLATE_BYTES) {
		throw new Error(`jobs_plan.promptTemplate exceeds ${MAX_PLAN_PROMPT_TEMPLATE_BYTES} bytes; shrink the template.`);
	}
	const matrix = record.matrix;
	if (!Array.isArray(matrix) || matrix.length === 0) throw new Error("jobs_plan.matrix must be a non-empty array.");
	if (matrix.length > MAX_PLAN_ROWS) throw new Error(`jobs_plan.matrix has ${matrix.length} rows; max is ${MAX_PLAN_ROWS}.`);
	const seen = new Set<string>();
	for (const [i, rowValue] of matrix.entries()) {
		assertObject(rowValue, `jobs_plan.matrix[${i}] must be an object.`);
		const row = rowValue as Record<string, unknown>;
		for (const key of Object.keys(row)) {
			if (!ALLOWED_ROW_KEYS.has(key)) throw new Error(`jobs_plan.matrix[${i}].${key} is not supported. Rows are frozen to id/name/cwd/vars.`);
		}
		assertString(row.id, `jobs_plan.matrix[${i}].id must be a non-empty string.`);
		const id = (row.id as string).trim();
		if (!SAFE_ID_RE.test(id)) throw new Error(`jobs_plan.matrix[${i}].id must use only letters, numbers, dot, underscore, or dash.`);
		if (seen.has(id)) throw new Error(`jobs_plan.matrix has duplicate id: ${id}`);
		seen.add(id);
		if (row.name !== undefined && (typeof row.name !== "string" || !(row.name as string).trim())) {
			throw new Error(`jobs_plan.matrix[${i}].name must be a non-empty string when provided.`);
		}
		if (row.cwd !== undefined && typeof row.cwd !== "string") {
			throw new Error(`jobs_plan.matrix[${i}].cwd must be a string when provided.`);
		}
		if (row.vars !== undefined) {
			assertObject(row.vars, `jobs_plan.matrix[${i}].vars must be an object.`);
			for (const [k, v] of Object.entries(row.vars as Record<string, unknown>)) {
				if (typeof v !== "string" && !isStringArray(v)) {
					throw new Error(`jobs_plan.matrix[${i}].vars.${k} must be a string or array of strings.`);
				}
			}
		}
	}
	if (record.concurrency !== undefined) {
		if (!Number.isInteger(record.concurrency) || (record.concurrency as number) < 1) {
			throw new Error("jobs_plan.concurrency must be a positive integer when provided.");
		}
	}
	if (record.synthesis !== undefined) {
		assertObject(record.synthesis, "jobs_plan.synthesis must be an object when provided. It is experimental metadata only, not a workflow language.");
		const synthesis = record.synthesis as Record<string, unknown>;
		for (const key of Object.keys(synthesis)) {
			if (key !== "mode" && key !== "instructions") throw new Error(`jobs_plan.synthesis.${key} is not supported. synthesis is experimental metadata only.`);
		}
		if (synthesis.mode !== undefined && synthesis.mode !== "parent" && synthesis.mode !== "report-only") {
			throw new Error("jobs_plan.synthesis.mode must be 'parent' or 'report-only' when provided.");
		}
		if (synthesis.instructions !== undefined && typeof synthesis.instructions !== "string") {
			throw new Error("jobs_plan.synthesis.instructions must be a string when provided.");
		}
	}
	const totalBytes = Buffer.byteLength(JSON.stringify(input), "utf-8");
	if (totalBytes > MAX_PLAN_TOTAL_INPUT_BYTES) {
		throw new Error(`jobs_plan input is ${totalBytes} bytes (limit ${MAX_PLAN_TOTAL_INPUT_BYTES}). Trim per-row vars or shrink the prompt template.`);
	}
}

function rowContext(row: JobsPlanRow): Ctx {
...
function substitute(template: string, ctx: Ctx, rowId: string, fieldLabel: string): string {
	return template.replace(TEMPLATE_RE, (_match, key) => {
		if (!(key in ctx)) {
			throw new Error(`jobs_plan.${fieldLabel} references unknown variable {{${key}}} for row ${rowId}.`);
		}
...
function expandMetadata(input: JobsPlanInput, ctx: Ctx, rowId: string): Record<string, string> | undefined {
	...
	out["jobs_plan.row_id"] = rowId;
	out["jobs_plan.batch_name"] = input.batchName;
	return out;
}

export function expandJobsPlan(input: JobsPlanInput): ExpandedJobsPlan {
	validateJobsPlanInput(input);
	const jobs: JobSpecInput[] = [];
	...
	const params: JobsToolParams = { jobs };
	...
	if (input.rerunOfJobIds) params.rerunOfJobIds = input.rerunOfJobIds;
	return { params, jobNames, rowIds };
}
...
export async function writePlanArtifact(batchDir: string, batchId: string, input: JobsPlanInput, expansion: ExpandedJobsPlan): Promise<string> {
...
export function buildPlanStartingText(input: JobsPlanInput, expansion: ExpandedJobsPlan, _cwd: string): string {
	const total = expansion.rowIds.length;
	const preview = expansion.rowIds.slice(0, 5).join(", ");
	const more = total > 5 ? `, …(+${total - 5})` : "";
	return [
		`JOBS plan starting · ${input.batchName} · preparing ${total} job${total === 1 ? "" : "s"}`,
		preview ? `rows: ${preview}${more}` : undefined,
	].filter(Boolean).join("\n");
}

export function decoratePlanResultText(baseText: string, planPath: string, input: JobsPlanInput): string {
