...
import type { JobActivityItem } from "./types.ts";
...
function activity(input: WorkerActivityContext & { kind: JobActivityItem["kind"]; label: string; detail?: string }): JobActivityItem {
  return {
...
function toolStartLabel(toolName: string, argsValue: unknown): string {
  ...
  if (toolName === "job_report") return "Submit job report";
...
function prettyToolName(toolName: string): string {
  if (toolName === "job_report") return "Job report";
...
function toolEndLabel(toolName: string, failed: boolean): string {
  if (toolName === "job_report") return failed ? "Job report failed" : "Job report submitted";
  return `${prettyToolName(toolName)} ${failed ? "failed" : "finished"}`;
}

export function extractWorkerActivity(record: unknown, state: WorkerActivityState, ctx: WorkerActivityContext): JobActivityItem | null {
...
function inferActivityRole(item: JobActivityItem): ActivityRole {
...
export function iconForActivity(item: JobActivityItem): string {
...
export function activitySummary(item: JobActivityItem): string {
  return truncate(stripAnsiAndControl(collapseWhitespace(item.label)), 96);
}

export function activityRole(item: JobActivityItem): ActivityRole {
  return inferActivityRole(item);
}

export function renderActivityCollapsedLine(item: JobActivityItem): string {
  return `│ Thinking ${iconForActivity(item)} ${activitySummary(item)} ·`;
}

export function renderActivitySummaryLines(items: JobActivityItem[], options: { maxItems?: number; header?: boolean; indent?: string } = {}): string[] {
...
export function renderActivityLine(item: JobActivityItem): string {
