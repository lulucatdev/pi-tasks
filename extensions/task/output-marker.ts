...
const MARKER_LINE_RE = /^\s*TASK_STATUS:\s*(completed|partial|blocked|error)\s*$/i;
const MARKER_ANYWHERE_RE = /TASK_STATUS:\s*(completed|partial|blocked|error)\b/gi;
...
export function inspectLegacyOutputMarker(output: string, options: { minNonMarkerChars?: number; stubPhrases?: string[] } = {}): LegacyMarkerInspection {
  ...
  if (anywhereCount !== markerLines.length) {
    warnings.push("TASK_STATUS appears outside a standalone marker line.");
  }

  if (markerLines.length === 0) {
    errors.push("No standalone TASK_STATUS marker line found.");
  } else if (markerLines.length > 1) {
    warnings.push(`Multiple TASK_STATUS marker lines found (${markerLines.length}).`);
  }

  const finalMarker = finalNonEmptyLine?.match(MARKER_LINE_RE) ?? null;
  if (!finalMarker) {
    warnings.push("Final non-empty line is not a TASK_STATUS marker.");
  }
